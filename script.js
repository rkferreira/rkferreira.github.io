...
async function fetchContributions() {
    ...
    try {
        ...
        // Fetch all events to find contributed repositories
        ...
        // Extract unique external repositories (not owned by the user)
        ...
        events.forEach(event => {
            if (event.repo && event.repo.name) {
                ...
                // Filter for external repositories (not owned by user)
        ...
        if (externalRepos.size === 0) {
            contributionsInfo.textContent = 'Fetching more contribution data...';
            
            // Try to fetch from search API for more comprehensive results
            const searchResponse = await fetch(
                `${GITHUB_API}/search/issues?q=author:${GITHUB_USERNAME}+type:pr&sort=updated&per_page=100`
            );
        ...
        // Fetch details for each external repository
        ...
        if (validRepos.length === 0) {
            contributionsInfo.innerHTML = `
                ...
                No recent external contributions found. This section shows repositories you've contributed to that are owned by others.
                <br><br>
                <small>Note: GitHub's public events API only shows recent activity (last 90 days). 
                For a complete history, you can use GitHub's advanced search or GraphQL API.</small>
            `;
        ...
        contributionsInfo.innerHTML = `
            ...
            Found <strong>${validRepos.length}</strong> external ${validRepos.length === 1 ? 'repository' : 'repositories'} you've contributed to recently!
        `;
